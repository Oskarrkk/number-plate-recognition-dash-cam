...
class DetectorMorph():
    ...
    def pre_process(self,frame):
        ...
        imgBlurred = cv2.GaussianBlur(imgMaxContrastGrayscale, (7, 7), 0)

        kernel_ver = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
        kernel_hor = np.array([[-1, -2, -1], [0, 0, 0], [1, 2, 1]])
        vertical = cv2.filter2D(imgBlurred, -1, kernel_ver)
        horizontal = cv2.filter2D(imgBlurred, -1, kernel_hor)
        comb = vertical + horizontal

        th4 = cv2.threshold(comb, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        return th4,vertical

    def filter_contour(self,threshold):
        threshold = cv2.morphologyEx(threshold, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (10, 1)))
    ...
    def find_plate(self,frame):
        ...
        candidates = []
        xy_candidates = []
        threshold, vertical = self.pre_process(frame)
